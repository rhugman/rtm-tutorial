...
| DSI | Plain `pyemu.emulators.DSI` in core (DSIAE = later add-on); transform choice taught as a beat (untransformed failure first); default IES solver — multimodal options deliberately dropped (empirically over-tighten emulator-space posteriors; evidence beat in part1_05); runstor workflow; fidelity check before conditioning, always |
...
- **part1_05_dsi_basics** *(draft; split from `xx_dsi` cells 1–37)* — train DSI on prior MC (minus truth); **fidelity check first** (held-out realisations; the transform beat: untransformed → negative concentrations → fix); then condition with PESTPP-IES on the emulator (runstor, default solver + dropped-multimodal evidence beat, noise ensemble); payoff: posterior vs prior forecast distribution (median + P95; "truer news, not better news") plus the 90 mg/L lens cell, seconds-vs-days compute contrast.
...
4. ~~`ies_multimodal_alpha` explanation cell~~ RESOLVED 2026-06-06 by controlled experiment: multimodal dropped everywhere (over-tightened the emulator-space posterior and excluded the truth; default solver matches the kNN reference). part1_05 carries the evidence.
