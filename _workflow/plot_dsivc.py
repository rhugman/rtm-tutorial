...
def fig_sweep_vs_dsivc(sweep_master, sweep_template, dsivc_master):
    ...
    a0.errorbar(af["cost"], af[P95], yerr=[(af[P95] - af[PMIN]).clip(lower=0),
                ...
                elinewidth=0.9, capsize=2, alpha=0.5, zorder=5)
    a0.plot(af["cost"], af[P95], color=C["black"], lw=1.8, marker="o", ms=5, zorder=6,
            label=f"DSIVC Pareto (P95 + stack min–max, n={len(af)})")
    if tpk is not None:
        a0.axhline(tpk, color=ROLE["truth"], ls="--", lw=1.4, label=f"truth ({tpk:.0f})")
    a0.set_xlabel(LBL["cost"])
    a0.set_ylabel("peak recovered SO$_4$ (mg/L)")
    ...
    # (b) SO4 vs f_treat: sweep scatter + Pareto members positioned by their injected f_treat, whiskered
    #     by the DSI stack range (min-max) -- the emulator's posterior spread at that treatment level.
    a1.scatter(s["f_treat"], s["peak_so4"], s=28, color=ROLE["prior"], edgecolor="none",
               alpha=0.45, zorder=3, label="full-model sweep")
    a1.errorbar(arc["f_treat"], arc[PMEAN], yerr=[(arc[PMEAN] - arc[PMIN]).clip(lower=0),
                (arc[PMAX] - arc[PMEAN]).clip(lower=0)], fmt="o", ms=4, color=ROLE["emulated"],
                ecolor=ROLE["emulated"], elinewidth=0.9, capsize=2, alpha=0.85, zorder=5,
