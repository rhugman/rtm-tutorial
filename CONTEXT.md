...
"How much sulfate will the supplied water carry, and how sure are we?" — a *minimization/design* framing: treatment cost scales with concentration, and the operator designs treatment capacity to the **P95 of peak SO₄**. The payoff tracked through every notebook is the forecast distribution (median and P95), prior vs posterior. The lesson of the case: conditioning shifts the forecast *up* (rate-aware prior median ≈ 81, 5–95% 70–93 → synthetic-spine posterior ≈ 89, 76–101, truth inside; the real-data capstone moves the same direction, harder and wider) — the value of data is truer news, not better news.
_Avoid_: framing the decision as threshold exceedance. Thresholds appear only as a one-cell illustrative risk lens (the 90 mg/L worked example: prior P ≈ 0.11 → posterior P ≈ 0.47 on the synthetic spine); the EU 250 mg/L standard is comfortably met and gets exactly one sentence (cost, not compliance, drives this decision). Notebooks compute these numbers live from the shipped artifacts — prose cites them, code never hardcodes them.
