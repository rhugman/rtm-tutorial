"""
Data Pattern Space Inversion (DPSI) emulator implementation.
This extends DSI with optional row-wise, group-aware scaling using
truth values from pst.observation_data for inverse transformation.
"""
...
class DPSI(Emulator):
    """
    DPSI: DSI variant that supports row-wise, group-aware scaling.
    ...
    def __init__(self,
        ...
        if isinstance(pst, pd.DataFrame):
        ...
        if self.rowwise_groups is not None and self.observation_data is not None:
            try:
                ...
                self.logger.statement("prefit truth row-wise scaler from pst.observation_data")
    ...
    def predict(self, pvals, pst: Pst = None, truth_selector=None):
        ...
        if self.rowwise_groups is None:
            if self.transforms is not None:
                return self.transformer_pipeline.inverse(sim_series)
            else:
    ...
    # The rest of APIs (prepare_pestpp, prepare_dsivc) can be inherited or copied
    # from DSI if needed. For brevity we don’t duplicate them here.
